...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  if (!product) return { title: `Product Not Found | ${META.SITE_NAME}` };
  ...
  if (error && !product) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <Link href={ROUTES.PRODUCTS} className="inline-flex items-center gap-2 bg-gray-900 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-gray-800 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="bg-white rounded-2xl border border-gray-100 p-10 flex items-center justify-center min-h-100">
          <div className="relative w-full max-w-sm aspect-square">
            <Image src={product.image} alt={product.title} fill sizes="(max-width: 1024px) 90vw, 40vw" className="object-contain" priority />
          </div>
        </div>

        <div className="flex flex-col">
          <span className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">{formatCategory(product.category)}</span>
          <h1 className="text-2xl sm:text-3xl font-black text-gray-900 leading-tight mb-4">{product.title}</h1>
          <div className="mb-5"><ProductRating rating={product.rating} showCount /></div>
          <div className="mb-6"><span className="text-4xl font-black text-gray-900">{formatPrice(product.price)}</span></div>
          ...
          <Link href={ROUTES.PRODUCTS} className="inline-flex items-center gap-1.5 mt-5 text-sm text-gray-500 hover:text-gray-900 transition-colors self-start">
